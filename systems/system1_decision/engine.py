...
class DecisionEngine:
    ...
    def _validate_state(self, state: State) -> list[float]:
        if hasattr(state.features, "values") and hasattr(state.features, "names"):
            values = list(state.features.values)
            names = list(state.features.names)
            label = "state.features.values"
        else:
            values = list(state.features)
            names = list(getattr(state, "feature_names", []))
            label = "state.features"

        if len(values) != self.feature_dim:
            raise ValueError(
                f"state feature length mismatch: expected {self.feature_dim}, received {len(values)}"
            )
        if len(names) != len(values):
            raise ValueError("state feature names must align with state features")
        features = [float(value) for value in values]
        self._validate_numeric_vector(features, label)
        return features
